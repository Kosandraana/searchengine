...
public class SearchResponseBuilder implements Runnable {
    ...
    private void defineLemmaFrequencies() {
        if (request.getSiteUrls().size() == 1) {
            site = Repo.siteRepo.findByUrlAndStatus(request.getSiteUrls().get(0), Site.INDEXED)
                    .orElse(null);
            if (site == null) {
                return;
            }
    ...
    private void defineRelevancesForFirstLemma(String lemma) {
        List<Indexx> indexes = site != null ?
                Repo.indexRepo.findAllByTextLemmaAndSite(lemma, site) :
                Repo.indexRepo.findAllByTextLemma(lemma);
        for (Indexx index : indexes) {
